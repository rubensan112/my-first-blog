...
def home_pagev3(request):
    posts = Pepito.objects.filter(published_date__lte=timezone.now()).order_by('published_date')
...
def post_detail(request, pk):
    post = get_object_or_404(Pepito, pk=pk)
...
def post_new(request):
    if request.method == "POST":
        form = PostForm1(request.POST) #request. Post son los datos que envia el usuario del formulario. Construimos el modelo del formulario Postform con esos datos.
        if form.is_valid():
...
def post_draft_list(request):
    posts = Pepito.objects.filter(published_date__isnull=True).order_by('created_date')
